...
class Siswa extends BaseController
{
        ...
        try {
            ...
            if (empty($user['foto_profil'])) {
                ...
                if (!$faceRecognitionResult) {
                    ...
                    if (!file_exists(FCPATH . 'uploads/profiles/' . $user['foto_profil'])) {
                        return $this->response->setJSON(['status' => 'error', 'message' => 'Verifikasi wajah gagal. Foto profil tidak ditemukan. Silakan upload ulang foto profil Anda.']);
                    } else {
                        return $this->response->setJSON(['status' => 'error', 'message' => 'Verifikasi wajah gagal. Wajah tidak dikenali atau tidak cocok dengan foto profil. Pastikan pencahayaan cukup dan wajah terlihat jelas.']);
                    }
                }
            }
    ...
    private function verifyFace($selfieData, $profilePhotoFilename)
    {
        try {
            ...
            // Gunakan 'py' untuk Python launcher di Windows
            $pythonPath = 'py';
            $command = $pythonPath . ' ' . ROOTPATH . "enhanced_face_compare.py " . escapeshellarg($profilePhotoPath) . " " . escapeshellarg($selfiePath) . " 2>&1";
            ...
            if ($result && isset($result['match'])) {
                ...
                if (isset($result['similarity'])) {
                    // Untuk produksi, kita gunakan threshold yang lebih tinggi (misalnya 0.6 atau 0.7)
                    $threshold = 0.6; // Threshold yang lebih tinggi untuk produksi
            ...
            if ($result && isset($result['error'])) {
                ...
                // Jika ada error library, kembalikan false
                return false;
            }
